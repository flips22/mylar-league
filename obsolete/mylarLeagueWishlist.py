'''
This script will add all of the series found in CBL reading list to mylar.

This script was based on TheMadman's script. The biggest change was that it now uses the simyan to search the comicvine api.  It works 
...
Results are output to "Wishlist-Output.csv" in the same directory as the script
Notes:
...
if TEST_MODE:
    ...
    OUTPUT_FILE = os.path.join(SCRIPT_DIR, "Wishlist-Output_new.csv")
else:
...
def main():
    ...
    print("Found %s series in CSV, %s new series in Wishlist" % (numExistingSeries,numNewSeries))
    ...
    print("Total Number of Series From CSV File: %s, New Series Added From Wishlist: %s,  Existing Series (Mylar): %s,  Missing Series (Mylar): %s,  New Matches (CV): %s, Unfound Series (CV): %s" % (numExistingSeries,numNewSeries,mylarExisting,mylarMissing,CVFound,CVNotFound))
